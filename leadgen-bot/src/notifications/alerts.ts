...
export type AlertType = 'twitter_error' | 'twitter_empty_session' | 'twitter_zero_results';
...
const ALERT_TEMPLATES: Record<AlertType, { title: string; defaultDesc: string }> = {
    twitter_error: {
...
export async function sendAlert(
    ...
    if (now - lastTime < ALERT_THROTTLE_MS) {
        console.log(`[Alerts] Throttling ${alertType} alert (last sent ${Math.round((now - lastTime) / 1000 / 60)} minutes ago)`);
        return false;
    }
...
export async function alertTwitterError(
    ...
    return sendAlert('twitter_error', 'The Twitter bot encountered a critical error', fields);
}
...
export async function alertTwitterEmptySession(
    ...
    const fields = [
        ...
        },
        {
            ...
            value: 'Please update TWITTER_SESSION environment variable with valid cookies',
            inline: false,
        },
    ];
...
export async function alertTwitterZeroResults(
    ...
    return sendAlert('twitter_zero_results', `No results found for query: ${query}`, fields);
}
