 ...
 */
async function analyzeWithGemini(title: string, content: string | null): Promise<JobAnalysis> {
 ...
 */
async function analyzeWithGLM(title: string, content: string | null): Promise<JobAnalysis> {
    const prompt = ANALYSIS_PROMPT
        .replace('{title}', title)
        .replace('{content}', content || '(no content)');
